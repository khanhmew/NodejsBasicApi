...
route.post("/login", function (req, res) {
  ...
  if ( isNotExistUsernameOrPassword ) {
    ...
    User.findOne({ username: usernameLogin })
      .then( accountFind => {
        if ( accountFind ) {
      ...
      .catch( err => {
        apiResponse(res, 400, false, err, null);
      });
  }
});
...
route.get("/friends", function (req, res) {
  ...
  if (typeof userForGetFriend == undefined) {
    ...
    const friendsForGet = listAccounts.filter((item) => {
      if (item.friends.includes(userForGetFriend.username)) return true;
      return false;
    });
...
route.post("/signup", (req, res) => {
  let isNotExistUsernameOrPassword =
    !usernameSignUp || !passwordSignUp || !fullnameSignUp;
  const {
  ...
  if (isNotExistUsernameOrPassword) {
    ...
    const userExists = listAccounts.find((user) => {
      return user.username === usernameSignUp;
    });
    if (userExists)
      apiResponse(
        res,
        400,
        false,
        "User Already Exists! Login or choose another username",
        null
      );
    else {
      let listFriendNew = [];
...
route.put("/user", function (req, res) {
  ...
  if (typeof user === "undefined") {
    ...
    const accountUpdate = listAccounts.find(
  ...
  if (typeof userDelete === "undefined") {
    ...
    if (userDelete.friends.includes(usernameForDelete)) {
      ...
      apiResponse(
        ...
        400,
        false,
        `You dont have ${userDelete} in your list friend!`,
        null
      );
    }
  }
...
route.delete("/user/:username", function (req, res) {
  ...
  if (typeof userDelete === "undefined") {
    ...
    if (usernameForDelete == userDelete.username) {
      const accountForDelete = listAccounts.find(
        (item) => item.username === usernameForDelete
      );
      if (accountForDelete) {
